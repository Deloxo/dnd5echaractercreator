...
function sumArray(array) {
    for (
        ...
        index < length; // The "for" -loop condition sum +=array[index++] // Add number on each iteration ); return sum; } function checkForRandom() { points=0; for (x=0; x < 6; x++) { var statsdice=[Math.floor(Math.random() * 6) + 1, Math.floor(Math.random() * 6) + 1, Math.floor(Math.random() * 6) + 1, Math.floor(Math.random() * 6) + 1]; statsdice.sort(); console.log(statsdice); statsdice.shift(); var result=sumArray(statsdice); console.log(result); switch (x) { case 0: stats.str=result; break; case 1: stats.dex=result; break; case 2: stats.con=result; break; case 3: stats.int=result; break; case 4: stats.wis=result; break; case 5: stats.cha=result; break; } } document.getElementById("str").innerHTML=stats.str; document.getElementById("dex").innerHTML=stats.dex; document.getElementById("con").innerHTML=stats.con; document.getElementById("int").innerHTML=stats.int; document.getElementById("wis").innerHTML=stats.wis; document.getElementById("cha").innerHTML=stats.cha; } function changeStat(move) { switch (move) { case 'strUp' : if (points> 0) {
        stats.str++; points--; document.getElementById("str").innerHTML = stats.str; document.getElementById("statbox").innerHTML = points;
    }
    break;
    case 'strDown':
    if (stats.str > 0) {
